...
const (
	...
	attrBlink
	attrWrap
	// attrWideDummy marks a cell as the trailing placeholder of a wide character.
	// Draw code should skip these cells entirely.
	attrWideDummy
)
...
// IsWideDummy returns true if this cell is a trailing placeholder for a
// double-width character and should be skipped during rendering.
func (g Glyph) IsWideDummy() bool {
	return g.Mode&attrWideDummy != 0
}
...
func (t *State) setChar(c rune, attr *Glyph, x, y int) {
	...
	t.lines[y][x] = *attr
	t.lines[y][x].Char = c
	// Clear the wide-dummy flag since this is a real character.
	t.lines[y][x].Mode &^= attrWideDummy
