...
func (t *State) parse(c rune) {
	...
	// For double-width characters, fill the next cell with a placeholder
	// so that cursor positioning and absolute addressing remain correct.
	if rw == 2 && t.cur.X+1 < t.cols {
	...
	if t.cur.X+rw < t.cols {
		t.moveTo(t.cur.X+rw, t.cur.Y)
	} else {
